---
title: "<div class='row'><div class='column left'>ANU Student Research Conference<br>Workshop: Introduction to R<span style='font-family:Verdana;font-size:50%'><br>https://bit.ly/2RvTNNr</span></div><div class='column right'>![](logo.png)</div></div>"
author: "Dr. Marcin Adamski marcin.adamski@anu.edu.au<br>&nbsp;&nbsp;Research School of Biology<br>&nbsp;&nbsp;John Curtin School of Medical Research"
...
We will get a taste of how to analyze data in R by working with data from (a fictional) project on Golden Orb-Weaver spiders. The research objective of this project is to compare spider populations living in different climate zones in Australia. We will compare body weight of a spider to the diameter of the web the spider weaves and asses if it is true that:
...
R Studio (written as RStudio without the space) is a free and open-source Integrated Development Environment (IDE) for R. You can download RStudio from [here](https://www.rstudio.com/products/rstudio/download/#download). RStudio makes our experience with R easier, but it is not necessary to program in R.
...
Almost all introductory R courses begin with using R as a calculator... We will do the same. Let's type the formulas (expressions) below and click \<Ctrl-ENTER\> on PC or \<Cmd-ENTER\> on Mac.

```{r}
#now, we use R as a calculator (order of operators is: (), ^, /, *, +, -):
2 + 2

#we can force the order using parenthesizes
3 + 5 * 2 ^ 2
(1 + 2) * 3
...
Our R-calculator can make plots!

```{r}
curve(sin, from = 0, to = 2 * pi, n = 100)
```

it's possible to use more complicated expression which uses the `x` as a variable
...
#use built-in function print() to print:
print("Hello World!")
...
myNumber  = 0.25      #numerical float or double
myInteger = 2L        #numerical integer or long
...
To list variables we use the _list_ function `ls()` (or we can consult the **Environment** panel in RStudio).
...
Do you understand how the `rm(list = ls())` worked?  
...
Let's give the logical operators a try:
...
As we all remember from our preschool algebra classes operators have their 'priority' or _precedence_ - the order in which they are used in calculations. The table below is a short summary of precedence order of the more common operators in R.
...
What has happened is called **type _coercion_** - the variable's data type was _coerced_ to the type suitable to store all the types of entered elements.
...
So, what will happen when we multiply a multi-element vector by a single-element vector representing scalar?
...
What has actually happened: The shorter vector _a_ was _recycled_ as many times as necessary to match length of the longer vector _b_. As _a_ has only 1 element it was an equivalent of multiplying each element of _b_ by scalar _a_.

#### Matrices

Now, lets quickly introduce a matrix: \
Matrix is a collection of elements arranged in a 2D rectangular layout. It is a vector where each element is a vector of the same length. All the matrix elements, _cells_ must be the same data-type (e.g. all numbers or all text, same as with vectors). Mostly, matrices in R are used to perform calculation with **_matrix algebra_** and they are numerical matrices.  \
...
We can check data type of a matrix. And the dimensions with functions `dim()`, `nrow()`, and `ncol()`.
...
We need to specify each column as a separate vector. All the vectors (columns) need the have the same number of elements (rows).
...
Data frame and matrix can be converted into each other with functions `as.matrix()`, to convert `data.frame` into `matrix` or `as.data.frame()` to do the opposite.  \


> Remember that `data.frame` is **heterogeneous** but `matrix` is **homogeneous**. Conversion of `data.frame` into `matrix` **may result in type coercion**.
...
Lets bring back our matrix _M_:
...
- Mild-Temperature Coastal - Jervis Bay, ACT
- Alpine - Cooma, NSW  
...
- population - location where the spider was collected (canberra, cooma, jervis),
...
Revealing `spider` internal structure
...
Now a real challenge: How to perform the same calculations as above, but using data from only the Canberra population?  \ We will subset rows of our data using indexing _by logical expression_. We will also access the column (web_dia or weight) indexing _by name of the column_.
...
Density plot:  
We will use context awareness of the `plot` function - it's ability to make different plots from objects of different classes:
...
## Test for statistical significance with _t_-test

To formally test for the pair-wise differences, we will use the **_t_-test** (two-sample unpaired; more on t-test [here](https://statistics.berkeley.edu/computing/r-t-tests)) to check if there is statistically significant difference between the populations. In statistical tests, the **_NULL_**, or **_H_~0~**  hypothesis is that there are **no differences**. To **reject the _NULL_ hypothesis** (meaning there are differences after all) we need the test's _p_-value (the probability that there indeed are no differences) to be below certain level called $\alpha$. Usually, in biology the level is expected to be equal or below 0.05 (meaning we are at least 95% sure there are differences), or 0.01 (meaning 99% of certainty).
...
```
In you opinion: Does spider weight differ between Canberra and Cooma populations?  \
...
What should we change in the statements above to compare the web diameters instead of spider weights?  
<br>
You have probably noticed that we were performing pairwise comparisons (each population vs. all others). If we are interested in general answer: Are there differences between the populations we can use the well-know 1-way ANOVA analysis, function `aov()`:

```{r}
spider.aov <- aov(web_dia ~ population, spider)
summary(spider.aov)
```

T-test and ANOVA, although powerful and commonly used by scientists assume that the data were sampled from normal distributions. How do we confirm that our data were indeed normally distributed? Formally, we should run a __test for normality__, e.g. Shapiro-Wilk Normality Test using the `shapiro.test()` function. Alternatively, we can visualize how well our data point fit the theoretical normal distribution with a Quantile-Quantile Plot using `qqnorm()` and `qqline()` functions:
...
The ggplot2 is a package, an extension to R. Before we start using it, we need to load the ggplot2 library. If we had not used ggplot2 ever before we need to install it first. We can use the `install.packages("ggplot2")` function or use the **Install** option from the **Packages** panel. In RStudio I suggest using the **Install** option and following the displayed dialog. Simply, follow the steps on the figure below:  
...
The statement above created an empty _container_ object for the plot. When creating this container we usually specify all the _components_ common to all the layers of the graph. The components are passed to `ggplot()` function as arguments. The common components are e.g. the data to be used and the plot coordinate system. The coordinates are passed as an _aesthetic_ `aes` object created with the `aes()` function.
...
Now, we have our _base_ ready and all what is left to do is to plot the data. This step is done with the _geometry_ layer(s). These layer(s) (there may be one or several) specify the form in which the data will be presented. The geometry components (among other components) are added to the ggplot object with the `+` operator.

### Scatter plots

To create an X-Y or a scatter plot, we use the geometry `geom_point()`.  
To ensure that the plot will appear in any situation it is better to explicitly print it with the `print()` function rather than rely on R ability to correctly direct the output. In next examples I will omit the `print()` function for simplicity.
...
Now, having the canvas ready, and conveniently saved under variable `g` we can add the box plot geometry.
...
<div class="footer"><br>End of Workshop: Introduction to R by Marcin Adamski on 17/09/2020  
